    ...
    while j < pages + 1:
        ...
        if not 'results' in json_responce:
            ...
            for i in range(len(answer_list)):
                ...
                content.append(
                    {'display-name': answer_list[i]['display_name'], 'id': answer_list[i]['id'], 'authorships': authors,
                     'abstract': text})
            cur.write(f'{cursor}\n')
            j += 1
    author_file = open("authors.json", 'w')
    print(author_set)
    json.dump(author_set, author_file)
    author_file.close()
    json.dump(content, json_file)
    json_file.close()


LoadAuthor('2020-01-01', 9999)
