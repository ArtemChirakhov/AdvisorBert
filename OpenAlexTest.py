...
def LoadAuthor(date, pages, cites=20):
    ...
    while j < pages + 1:
        print(j)
        #добавь обратно ",cited-by-count:>{cites}" в url
        url = f"https://api.openalex.org/works?filter=from_publication_date:{date}&per-page=200&select=display_name,id,authorships,abstract_inverted_index&cursor={cursor}"
        ...
        if not 'results' in json_responce:
            ...
            for i in range(len(answer_list)):
                ...
                json.dump({'display-name': answer_list[i]['display_name'], 'id': answer_list[i]['id'], 'authorships': authors, 'abstract': text}, jj)
            cur.write(f'{cursor}\n')
            j += 1
